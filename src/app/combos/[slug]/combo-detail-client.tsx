        ...
        >
                  ...
                  <li key={item.id} className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-700 relative overflow-hidden shrink-0">
                      {item.product.imageUrl ? (
                        ...
                        <div className="flex items-center justify-center h-full text-slate-400 text-xs">­ƒôª</div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">
                        {item.quantity > 1 ? `${item.quantity}├ù ` : ""}
                        {item.product.name}
                      </p>
                    </div>
          ...
          {combo.surpriseGift && (
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 border border-purple-200 dark:border-purple-700 rounded-2xl p-5 flex gap-3">
              ...
              <div>
                <p className="font-bold text-purple-800 dark:text-purple-200 text-sm">­ƒÄü Regalo sorpresa incluido</p>
                ...
                <p className="text-xs text-purple-400 dark:text-purple-500 mt-2">
                  El regalo se revela en el correo de confirmaci├│n una vez completado tu pedido.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            {isLowStock && (
              <span className="inline-flex items-center gap-1.5 text-orange-600 dark:text-orange-400 font-semibold">
                ...
                ┬íSolo quedan {combo.stock} disponibles!
              </span>
            )}
            {isOutOfStock && (
            ...
            {combo.soldCount > 0 && !isOutOfStock && (
              <span className="text-slate-500 dark:text-slate-400">­ƒöÑ {combo.soldCount} personas ya lo compraron</span>
            )}
          </div>
          ...
          <ul className="grid grid-cols-2 gap-2 text-xs text-slate-500 dark:text-slate-400">
            {["Env├¡o a todo Colombia", "Pago seguro Wompi", "Garant├¡a incluida", "Soporte post-venta"].map((b) => (
