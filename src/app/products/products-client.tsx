...
// --- Pillar 3: Shimmer skeleton card ---
  ...
  return (
    ...
    >
      ...
      <div className="p-3 space-y-2.5">
        <div className="skeleton-line w-4/5 relative overflow-hidden">
          <div className="skeleton-shimmer" />
        </div>
        <div className="skeleton-line w-3/5 relative overflow-hidden">
          <div className="skeleton-shimmer" />
        </div>
        <div className="h-8 rounded-xl bg-slate-200 dark:bg-slate-800 mt-1 relative overflow-hidden">
          <div className="skeleton-shimmer" />
        </div>
      </div>
    </div>
...
// Dynamic-width progress bar via data attribute + CSS variable
function ProgressBar({ value, total }: { value: number; total: number }) {
  const pct = total > 0 ? Math.min(100, Math.round((value / total) * 100)) : 0;
  return (
    <div
      className="h-full rounded-full bg-gradient-to-r from-[#0A2A66] to-[#2E5FA7] transition-all duration-500"
      data-pct={pct}
...
export default function ProductsClient({
  ...
  // Skip initial double-fetch when SSR provided fresh products
  const isFirstMount = useRef(true);
  // Sentinel element for IntersectionObserver
  const sentinelRef = useRef<HTMLDivElement>(null);
  // Stable ref to avoid stale closures in IO callback
  ...
  // Trigger fetch when filters change; skip very first mount if SSR data is fresh
  useEffect(() => {
    if (isFirstMount.current) {
      ...
      if (initialProducts.length > 0 && !urlHasFilters) return;
    }
    fetchProductsFromServer(1, false);
  }, [fetchProductsFromServer, initialProducts.length]);

  // --- Pillar 2: IntersectionObserver sentinel ---
  useEffect(() => {
    if (!hasMore) return;
  ...
  // When page increments (via IO), fetch and append
  useEffect(() => {
    if (page <= 1) return;
    fetchProductsFromServer(page, true);
  ...
  const modelSuggestions = useMemo(() => {
    ...
    return Array.from(counts.entries()).sort((a, b) => b[1] - a[1]).map(([t]) => t).slice(0, 12);
  }, [products]);

  const areFiltersActive = useMemo(
    () => !!searchTerm || selectedCategory !== "all" || selectedSize !== "all" || selectedColor !== "all" || selectedModel !== "all" || !!minPrice || !!maxPrice || sortBy !== "relevance",
      ...
      <div className="sticky top-[68px] z-30 bg-white/95 dark:bg-[#070617]/98 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 py-3">
            ...
            <button
              type="button"
              aria-label={areFiltersActive ? `Filtros activos: ${activeFilterCount}` : "Abrir filtros"}
              onClick={() => setShowMobileFilters(true)}
              className={`md:hidden flex items-center gap-1.5 pl-3 pr-4 py-2.5 rounded-xl text-sm font-semibold transition-colors ${areFiltersActive ? "bg-[#0A2A66] text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700"}`}
            >
          ...
          <div className="flex gap-2 overflow-x-auto pb-3 no-scrollbar">
            <button
              ...
              className={`flex-none px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all duration-200 ${selectedCategory === "all" ? "bg-[#0A2A66] text-white shadow-md" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700"}`}
            >
              Todos
            </button>
            {PRODUCT_CATEGORIES.map((cat) => (
              <button
                ...
                className={`flex-none px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all duration-200 ${selectedCategory === cat ? "bg-[#0A2A66] text-white shadow-md" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700"}`}
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
        ...
        <div className="hidden md:flex flex-wrap items-center gap-2 mb-5">
          ...
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-xl px-3 py-2 border border-transparent text-sm">
            ...
            <input type="number" aria-label="Precio m�nimo" placeholder="M�n" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} onBlur={() => handlePriceChange(minPrice, maxPrice)} className="w-16 text-xs bg-transparent outline-none text-slate-700 dark:text-slate-300 placeholder-slate-400" />
            <span className="text-slate-400 mx-1">�</span>
            <input type="number" aria-label="Precio m�ximo" placeholder="M�x" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} onBlur={() => handlePriceChange(minPrice, maxPrice)} className="w-16 text-xs bg-transparent outline-none text-slate-700 dark:text-slate-300 placeholder-slate-400" />
          </div>
          {areFiltersActive && (
            <button onClick={handleResetFilters} className="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-500 hover:bg-red-500 hover:text-white transition-all">
        ...
        {selectedCategoryLabel && (
          <div className="mb-5 rounded-2xl bg-gradient-to-r from-[#0A2A66]/8 to-[#2E5FA7]/5 dark:from-[#0A2A66]/25 dark:to-[#2E5FA7]/10 border border-[#0A2A66]/15 dark:border-[#2E5FA7]/20 px-5 py-4">
            ...
            {selectedCategoryDescription && <p className="text-sm text-slate-600 dark:text-slate-400 mt-0.5">{selectedCategoryDescription}</p>}
          </div>
        )}
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            {searchTerm && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#0A2A66]/10 dark:bg-[#2E5FA7]/20 text-[#0A2A66] dark:text-[#5B9BD5] text-xs font-semibold border border-[#0A2A66]/15 dark:border-[#2E5FA7]/25">
                ...
                <button type="button" aria-label="Quitar b�squeda" onClick={() => updateURLParams({ q: "" })}><XMarkIcon className="w-3 h-3 hover:text-red-500" /></button>
              </span>
            )}
            {selectedSize !== "all" && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#0A2A66]/10 dark:bg-[#2E5FA7]/20 text-[#0A2A66] dark:text-[#5B9BD5] text-xs font-semibold border border-[#0A2A66]/15 dark:border-[#2E5FA7]/25">
        ...
        {/* Pillar 3: Initial skeleton */}
        {loading && products.length === 0 && <SkeletonGrid count={8} />}
        ...
        <AnimatePresence mode="wait">
          {visibleProducts.length > 0 ? (
            ...
            >
              {/* Pillar 4: lazy-loaded images via Next.js Image (handled in ProductCard) */}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              {/* Pillar 2: Infinite scroll - skeleton row while loading more */}
              {loadingMore && (
                <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              {/* Pillar 2: Invisible sentinel watched by IntersectionObserver */}
              {hasMore && (
                <>
                  <div ref={sentinelRef} className="w-full h-1 mt-4" aria-hidden="true" />
                  <div className="mt-4 flex flex-col items-center gap-1.5">
                    <div className="w-48 h-1 rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden">
                      <ProgressBar value={products.length} total={totalCount} />
                    </div>
                    <p className="text-xs text-slate-400 dark:text-slate-600">
                      {products.length} de {totalCount} productos cargados
                    </p>
                  </div>
                </>
              )}
              ...
              {!hasMore && products.length > 0 && !loading && (
                <div className="mt-10 flex flex-col items-center gap-2 text-center pb-8">
                  ...
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                    Has visto los {products.length} productos del cat�logo
                  </p>
                </div>
              )}
            </motion.div>
            ...
            >
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm text-sm leading-relaxed">
                Prueba cambiando los filtros o el t�rmino de b�squeda.
              </p>
              {areFiltersActive && (
                <button
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 dark:border-slate-800">
                ...
                <button type="button" aria-label="Cerrar filtros" onClick={() => setShowMobileFilters(false)} className="w-8 h-8 flex items-center justify-center rounded-full text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
              ...
              <div className="max-h-[62vh] overflow-y-auto px-5 py-4 space-y-4">
                ...
                <FilterSelect
                  ...
                  isMobile
                  formatOptionLabel={(v) => ({ relevance: "Relevancia", "price-asc": "Menor precio", "price-desc": "Mayor precio", newest: "M�s recientes" } as Record<string, string>)[v] ?? v}
                />
                <div>
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Rango de precio</p>
                  <div className="flex gap-2">
                    <input type="number" aria-label="Precio m�nimo" placeholder="M�nimo" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} onBlur={() => handlePriceChange(minPrice, maxPrice)} className="flex-1 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-[#0A2A66] dark:focus:border-[#2E5FA7]" />
                    <input type="number" aria-label="Precio m�ximo" placeholder="M�ximo" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} onBlur={() => handlePriceChange(minPrice, maxPrice)} className="flex-1 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-[#0A2A66] dark:focus:border-[#2E5FA7]" />
                  </div>
                </div>
              </div>
              <div className="px-5 py-4 border-t border-slate-100 dark:border-slate-800 flex gap-3">
                {areFiltersActive && (
                  <button onClick={handleResetFilters} className="flex-1 py-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    Limpiar
                  </button>
                )}
                <button
...
function FilterSelect({
  label, value, onChange, options, isMobile = false, formatOptionLabel,
}: {
  label: string;
  value: string;
  ...
  const base = "w-full text-sm border focus:ring-2 focus:ring-[#0A2A66] outline-none";
  const desktop = "sm:w-auto pl-4 pr-10 py-3 rounded-xl bg-white/50 dark:bg-slate-800/50 text-slate-800 dark:text-slate-200 border-slate-300 dark:border-slate-700 appearance-none";
  const mobile = "p-3 rounded-lg bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700";

  return (
    <div className="relative">
      <select aria-label={label} value={value} onChange={onChange} className={`${base} ${isMobile ? mobile : desktop}`}>
        <option value="all">{label}</option>
        {options.map((o) => (
          <option key={o} value={o}>{formatOptionLabel ? formatOptionLabel(o) : o}</option>
        ))}
      </select>
